...
group = "com.dremio.oauth2"

description = "Dremio OAuth2 Agent"

ext { set("mavenName", "Dremio OAuth2 Agent") }
...
tasks.register<JavaExec>("generateDocs") {
  ...
  mainClass.set("com.dremio.oauth2.agent.docs.DocumentationGenerator")
  classpath = sourceSets.getByName("docs").runtimeClasspath

  val inputFile = project.file("src/main/java/com/dremio/oauth2/agent/OAuth2AgentConfig.java")
...
rootProject.tasks.named("spotlessMarkdown") { dependsOn(":oauth2-agent:generateDocs") }

rootProject.tasks.named("rat") { dependsOn(":oauth2-agent:generateDocs") }
