...
class ActorTokenSupplierTest {
  ...
  void testSupplyActorTokenAsyncStatic() {
    OAuth2AgentConfig config =
        createMainConfig("actor-token", null, TokenTypeURI.ID_TOKEN, Map.of());
  ...
  void testSupplyActorTokenAsyncNull() {
    OAuth2AgentConfig config = createMainConfig(null, null, TokenTypeURI.ACCESS_TOKEN, Map.of());
    try (ActorTokenSupplier supplier = createSupplier(config)) {
      CompletionStage<AccessToken> stage = supplier.supplyTokenAsync();
  ...
  private static ActorTokenSupplier createSupplier(OAuth2AgentConfig config) {
