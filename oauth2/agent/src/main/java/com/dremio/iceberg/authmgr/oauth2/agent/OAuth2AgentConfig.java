...
import static com.dremio.iceberg.authmgr.oauth2.agent.config.BasicConfig.CLIENT_AUTH;
import static com.dremio.iceberg.authmgr.oauth2.agent.config.JwtClientAuthConfig.ALGORITHM;
import static com.dremio.iceberg.authmgr.oauth2.agent.config.JwtClientAuthConfig.PRIVATE_KEY;
import static com.dremio.iceberg.authmgr.oauth2.agent.config.ResourceOwnerConfig.PASSWORD;
import static com.dremio.iceberg.authmgr.oauth2.agent.config.ResourceOwnerConfig.USERNAME;
...
public interface OAuth2AgentConfig {
  ...
  /**
   * Creates an {@link OAuth2AgentConfig} from the environment and the given catalog session
   * properties map.
   *
   ...
   */
  static OAuth2AgentConfig from(Map<String, String> properties) {
    ...
    SmallRyeConfig smallRyeConfig =
        new SmallRyeConfigBuilder()
            ...
            .withSources(source)
            .withMapping(OAuth2AgentConfig.class)
            .build();
    OAuth2AgentConfig config = smallRyeConfig.getConfigMapping(OAuth2AgentConfig.class);
    config.validate();
    return config;
  }
