...
public class OAuth2Manager extends RefreshingAuthManager {
  ...
  public AuthSession initSession(RESTClient initClient, Map<String, String> initProperties) {
    ...
    initializeFromConfig(initConfig);
    initSession = new OAuth2Session(initConfig, refreshExecutor(), httpClient);
    return initSession;
  }
  ...
  public AuthSession catalogSession(
    ...
    initializeFromConfig(catalogConfig);
    ...
    if (initSession != null && catalogConfig.equals(initSession.getConfig())) {
      ...
      catalogSession = new OAuth2Session(catalogConfig, refreshExecutor(), httpClient);
    }
    initSession = null;
  ...
  private AuthSession maybeCacheSession(AuthSession parent, Map<String, String> tableProperties) {
    ...
    return childConfig.equals(parentConfig)
        ...
        : sessionCache.cachedSession(
            childConfig, k -> new OAuth2Session(childConfig, refreshExecutor(), httpClient));
  }
  ...
  public AuthSession tableSession(RESTClient sharedClient, Map<String, String> properties) {
    ...
    return sessionCache.cachedSession(
        config, k -> new OAuth2Session(config, refreshExecutor(), httpClient));
  }
  ...
  private void initializeFromConfig(OAuth2Config config) {
    if (sessionCache == null) {
      sessionCache = sessionCacheFactory.apply(name, config);
    }
    if (httpClient == null) {
      httpClient = config.getSystemConfig().getHttpClientType().newHttpClient();
    }
  }
