...
public final class OAuth2Agent implements Closeable {
  ...
  private final OAuth2Config settings;
  ...
  public OAuth2Agent(
      OAuth2Config settings, ScheduledExecutorService executor, HTTPRequestSender httpClient) {
    this.settings = settings;
    this.executor = executor;
    this.flowFactory = FlowFactory.of(settings, executor, httpClient);
    name = settings.getSystemConfig().getAgentName();
    clock = settings.getSystemConfig().getClock();
    ...
    var requiresUserInteraction =
        ConfigUtils.requiresUserInteraction(settings.getBasicConfig().getGrantType());
  ...
  TokensResult getCurrentTokens() {
    try {
      Duration timeout = settings.getBasicConfig().getTimeout();
  ...
  CompletionStage<TokensResult> fetchNewTokens() {
    ...
    return ConfigUtils.requiresUserInteraction(settings.getBasicConfig().getGrantType())
  ...
  private void maybeScheduleTokensRenewal(@Nullable TokensResult currentTokens) {
    if (!settings.getTokenRefreshConfig().isEnabled()) {
      LOGGER.debug(
    ...
    boolean idle =
        timeSinceLastAccess.compareTo(settings.getTokenRefreshConfig().getIdleTimeout()) > 0;
  ...
  private Duration nextTokenRefresh(@Nullable TokensResult currentTokens, Instant now) {
    Duration minRefreshDelay = settings.getTokenRefreshConfig().getMinRefreshDelay();
    if (currentTokens == null || currentTokens.isExpired(now)) {
      return minRefreshDelay;
    }
    Instant expirationTime = currentTokens.getExpirationTime();
    if (expirationTime == null) {
      Duration defaultLifespan = settings.getTokenRefreshConfig().getAccessTokenLifespan();
      maybeWarn(
    ...
    Duration delay =
        Duration.between(now, expirationTime)
            .minus(settings.getTokenRefreshConfig().getSafetyWindow());
  ...
  private void wakeUp(Instant now) {
    ...
    if (currentTokens == null
        || currentTokens.isExpired(now.plus(settings.getTokenRefreshConfig().getSafetyWindow()))) {
