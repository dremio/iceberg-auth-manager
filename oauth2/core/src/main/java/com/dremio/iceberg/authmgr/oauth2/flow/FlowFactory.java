...
public abstract class FlowFactory implements AutoCloseable {
  ...
  public void close() {
    if (getConfig().getBasicConfig().getGrantType() == GrantType.TOKEN_EXCHANGE) {
  ...
  protected EndpointProvider getEndpointProvider() {
    return EndpointProvider.of(getConfig(), getHttpClient());
  }
  ...
  protected SubjectTokenSupplier getSubjectTokenSupplier() {
    return getConfig().getBasicConfig().getGrantType() != GrantType.TOKEN_EXCHANGE
        ? null
        : SubjectTokenSupplier.of(getConfig(), getExecutor(), getHttpClient());
  }
  ...
  protected ActorTokenSupplier getActorTokenSupplier() {
    return getConfig().getBasicConfig().getGrantType() != GrantType.TOKEN_EXCHANGE
        ? null
        : ActorTokenSupplier.of(getConfig(), getExecutor(), getHttpClient());
  }
  ...
  private AbstractFlow.Builder<? extends Flow, ?> newTokenRefreshFlowBuilder() {
    return ImmutableRefreshTokenFlow.builder();
  }
}
