...
public interface BasicConfig {
  ...
  List<GrantType> SUPPORTED_GRANT_TYPES =
      List.of(
   ...
   * ClientAuthenticationMethod#CLIENT_SECRET_BASIC} if the client is private, or {@link
   * ClientAuthenticationMethod#NONE} if the client is public.
   *
  ...
  default BasicConfig validate() {
    ...
    validator.check(
        SUPPORTED_GRANT_TYPES.contains(getGrantType()),
        GRANT_TYPE,
        "grant type must be one of: %s",
        SUPPORTED_GRANT_TYPES.stream()
