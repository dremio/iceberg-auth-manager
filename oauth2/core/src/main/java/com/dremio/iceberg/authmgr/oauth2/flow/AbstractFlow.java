...
abstract class AbstractFlow implements Flow {
  ...
  interface Builder<F extends AbstractFlow, B extends Builder<F, B>> {
    ...
    @CanIgnoreReturnValue
    B httpClient(HTTPRequestSender httpClient);
  ...
  abstract HTTPRequestSender getHttpClient();
  ...
  TokenRequest.Builder newTokenRequestBuilder(AuthorizationGrant grant) {
    ...
    return clientAuthentication == null
        ? new TokenRequest.Builder(tokenEndpoint, getConfig().getBasicConfig().getClientId(), grant)
        : new TokenRequest.Builder(tokenEndpoint, clientAuthentication, grant);
  }
  ...
  HTTPResponse sendAndReceive(HTTPRequest request) {
    try {
      LOGGER.debug(
          ...
          request.getURI());
      return request.send(getHttpClient());
    } catch (Exception e) {
  ...
  ClientAuthentication createClientAuthentication() {
    ...
    } else if (method.equals(CLIENT_SECRET_BASIC)) {
      return new ClientSecretBasic(
          getConfig().getBasicConfig().getClientId(),
  ...
  private JWTAssertionDetails createJwtAssertionDetails(URI tokenEndpoint) {
    Issuer issuer =
        getConfig().getClientAssertionConfig().getIssuer().isPresent()
            ...
            : new Issuer(getConfig().getBasicConfig().getClientId());
    Subject subject =
        getConfig().getClientAssertionConfig().getSubject().isPresent()
            ? getConfig().getClientAssertionConfig().getSubject().get()
            : new Subject(getConfig().getBasicConfig().getClientId().getValue());
    Audience audience =
