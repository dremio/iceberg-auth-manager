...
class AuthorizationCodeFlow extends AbstractFlow {
            ...
            .orElseGet(
                () ->
                    URI.create(
                            String.format(
                                "http://%s:%d/%s",
                                bindHost, server.getAddress().getPort(), contextPath))
                        .normalize());
    URI authEndpoint = endpointResolver.getResolvedAuthorizationEndpoint();
    authorizationUri =
        new UriBuilder(authEndpoint.resolve("/"))
            ...
            .queryParam(
                "scope",
                spec.getBasicConfig().getScopes().stream()
                    .reduce((a, b) -> a + " " + b)
                    .orElse(null))
  ...
  private String extractAuthorizationCode(HttpExchange exchange) {
    ...
    Map<String, String> params = FlowUtils.parseQueryString(exchange.getRequestURI().getQuery());
